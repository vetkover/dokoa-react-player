...
const link = "https://drive.google.com/uc?export=download&id=1yEUV3hJaEz8ImpWH_PsG2cHARPfPRPxb";
const links = [
    {
        quality: 1080,
...
const PlayerModule = () => {
    ...
    let rightRewind = (e) => {
        ...
        if (e.detail > 1 && isPhone) {
            if (!window.window.isPlayed) { Play() }
            dokoaPlayer.current.currentTime += 5;
        }
    }
    ...
    let Play = (status) => {
        ...
        if (status == "updateQuality") {
            if (dokoaPlayer.current.currentTime > 0 && !dokoaPlayer.current.paused && !dokoaPlayer.current.ended && dokoaPlayer.current.readyState > 2 ||window.isPlayed) {
    ...
    const settingsOptionChange = (option) => {
        ...
        setWindowStatus({
            last: windowStatus.active, active: option,
        })
    ...
    const setQuallityVideo = (el) => {
        if (el === 'ImMetaData') {
            ...
            let duration = timeInMinutes + ':' + timeInSeconds;
            setTimeData({
                current: duration, ...timeData, 
            })

        } else {
            setTimeData({
                ...timeData, onChangeQuality: dokoaPlayer.current.currentTime,
            });
    ...
    function SettingsQualitySwitch() {
        ...
        links.map((el => {
            return (
                <button className="settingsOption" key={el.quality} onClick={function () { setQuallityVideo(el) }}>
    ...
    const SettingsSpeedSwitch = () => {
        const speedTable = ["2", "1.75", "1.50", "1.25", "1", "0.75", "0.50", "0.25"]
        return (
            speedTable.map((speed, index) => {
                return (
                    <button className="settingsOption" key={speed} onClick={function () { setVideoSpeed(speed) }}>
                        <div className="settignsIco">
                        </div>
    ...
    return (
        <div >
            <div className="dokoaPlayer" ref={dokoaParent} onMouseMove={interfaceVisible} onWheel={interfaceVisible} onKeyDown={keyReg}>
                ...
                <div className="settingsParent playerInterface" ref={settingsParent}>
                    ...
                    <div className="settingsContainer">
                        ...
                        <div className="settingsMainWndow" ref={settingsMainWndow}>
                            <button className="settingsOption" onClick={function () {
                            ...
                            <button className="settingsOption" onClick={function () {
                                ...
                                <div className="settignsIco">
                                    <svg width="38px" height="40px" viewBox="0 0 24 24" fill="white">
